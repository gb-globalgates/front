...
// Keep rendered HTML safe when we build tag-search results.
...
function setupAccountMenu() {
    ...
    if (!layersRoot || !accountCard || !accountMenuPopup || !accountLogoutButton) {
        return;
    }
    ...
    function updateAccountMenuPosition() {
        ...
        const viewportPadding = 16;
        const popupWidth = Math.min(216, window.innerWidth - viewportPadding * 2);
...
function setupPostMoreMenus() {
    ...
    const postMoreIcons = {
        ...
        block:
            '<svg viewBox="0 0 24 24" aria-hidden="true" class="post-more-menu__icon"><g><path d="M12 3.75c-4.55 0-8.25 3.69-8.25 8.25 0 1.92.66 3.68 1.75 5.08L17.09 5.5C15.68 4.4 13.92 3.75 12 3.75zm6.5 3.17L6.92 18.5c1.4 1.1 3.16 1.75 5.08 1.75 4.56 0 8.25-3.69 8.25-8.25 0-1.92-.65-3.68-1.75-5.08zM1.75 12C1.75 6.34 6.34 1.75 12 1.75S22.25 6.34 22.25 12 17.66 22.25 12 22.25 1.75 17.66 1.75 12z"></path></g></svg>',
        report:
            '<svg viewBox="0 0 24 24" aria-hidden="true" class="post-more-menu__icon"><g><path d="M3 2h18.61l-3.5 7 3.5 7H5v6H3V2zm2 12h13.38l-2.5-5 2.5-5H5v10z"></path></g></svg>',
    };
    ...
    function syncPostMoreMenu(button, menu) {
        const items = Array.from(menu.querySelectorAll(".post-more-menu__item"));
...
function setupComposerModal() {
    ...
    function closeComposerModal() {
        ...
        if (typeof composerSection.__closeComposerLocationModal === "function") {
...
function setupComposerToolbar() {
    ...
    const locationModalDelete =
        document.getElementById("composerLocationDelete");
    const locationModalApply =
        document.getElementById("composerLocationApply");
    ...
    function renderMediaEditor() {
        ...
        if (mediaNextButton) {
            mediaNextButton.disabled =
                activeComposerMediaIndex >= pendingComposerMediaEdits.length - 1;
        }
    }
    ...
    locationModalList?.addEventListener("click", (event) => {
        const locationItem = event.target.closest(".tweet-modal__location-item");
...
function setupComposerDraftPanel() {
    ...
    async function buildComposerSnapshot() {
        ...
        const mediaEdits = Array.isArray(
            ...
            : [];
        const attachments =
            await serializeComposerAttachmentsForDraft(
                attachmentFiles,
                mediaEdits,
            );
...
function setupReplyModal() {
    ...
    function getCurrentAccountAvatar() {
        const avatarText =
            ...
            document.getElementById("accountName")?.textContent?.trim()?.charAt(0) ||
            "나";
    ...
    function renderReplyAttachment() {
        if (!replyAttachmentPreview || !replyAttachmentMedia || !replyFileInput) {
            return;
        }
        if (attachedReplyFiles.length === 0) {
        ...
        if (replyMediaUploadButton) {
            replyMediaUploadButton.disabled =
                attachedReplyFiles.length >= maxReplyImages && !isReplyVideoSet();
        }
    ...
    function handleReplyFileChange(event) {
        ...
        if (pendingAttachmentEditIndex !== null) {
            if (replacement.type.startsWith("video/")) {
                ...
                const editable = isReplyVideoSet() ? [] : [...attachedReplyFiles];
    ...
    function renderTagResults(users) {
        ...
        replyTagResults.innerHTML = users
            .map((user) => {
                const isSelected = pendingTaggedUsers.some((entry) => entry.id === user.id);
    ...
    function renderMediaEditor() {
        if (!replyMediaPreviewImage || !replyMediaAltInput || !replyMediaAltCount) {
            return;
        }
    ...
    function populateReplyModal(button) {
        ...
        const name = getTextContent(postCard.querySelector(".postName")) || "사용자";
        const handle = getTextContent(postCard.querySelector(".postHandle")) || "@user";
        const time = getTextContent(postCard.querySelector(".postTime")) || "방금 전";
        const text = getTextContent(postCard.querySelector(".postText")) || "본문이 없습니다.";
        const avatar =
            postCard.querySelector(".postAvatarImage")?.getAttribute("src") ||
            buildReplyAvatarDataUri(
                getTextContent(postCard.querySelector(".postAvatar")) ||
    ...
    function canCloseReplyModal() {
        return (!hasReplyEditorText() && !hasReplyAttachment()) || window.confirm("게시물을 삭제하시겠어요?");
    }
    ...
    function areAllDraftItemsSelected() {
        ...
        return items.length > 0 && draftPanelState.selectedItems.size === items.length;
    }
    ...
    function buildDraftCheckbox(selected) {
        ...
        checkbox.innerHTML = '<svg viewBox="0 0 24 24"><g><path d="M9.86 18a1 1 0 01-.73-.31l-3.9-4.11 1.45-1.38 3.2 3.38 7.46-8.1 1.47 1.36-8.19 8.9A1 1 0 019.86 18z"></path></g></svg>';
        return checkbox;
    }
    ...
    function renderDraftPanel() {
        ...
        if (draftEmptyBody) {
            draftEmptyBody.textContent = "아직 게시할 준비가 되지 않았나요? 임시저장해 두고 나중에 이어서 작성하세요.";
        }
        if (draftFooter) {
            draftFooter.hidden = !draftPanelState.isEditMode;
        }
        ...
        if (draftConfirmDesc) {
            draftConfirmDesc.textContent = "이 작업은 취소할 수 없으며 선택한 전송하지 않은 게시물이 삭제됩니다.";
        }
    }
    ...
    function trapFocus(event) {
        ...
        const focusable = qAll('button:not([disabled]), input:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])').filter(
    ...
    replyEmojiButton?.addEventListener("mousedown", (event) => event.preventDefault());
    ...
    replyEmojiContent?.addEventListener("mousedown", (event) => {
        if (event.target.closest(".tweet-modal__emoji-option, .tweet-modal__emoji-clear")) {
            event.preventDefault();
        }
    });
    ...
    replyTagSearchForm?.addEventListener("submit", (event) => event.preventDefault());
